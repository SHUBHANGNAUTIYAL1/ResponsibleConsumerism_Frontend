...
function Clothing() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.get('https://responsibleconsumerism-backend.onrender.com/api/product/cloths');
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="flex flex-col gap-12 px-20 py-10 bg-slate-100">
        <div>
          <h1 className="text-[40px] font-bold font-serif">Clothings</h1>
        </div>
          ...
          {Products.map((sale, index) => (
            <div className='px-5' key={index}>
              <div 
                ...
                className="flex flex-col justify-between items-center bg-white w-[270px] p-4 shadow-md m-2 border rounded-sm border-gray-200 h-[300px]"
              >
